module GitolitePublicKeysHelper
  def gitolite_public_keys_status_options_for_select(user, selected)
    ...
    options_for_select([[l(:label_all), nil], 
                        ["#{l(:status_active)} (#{key_count_by_active[1].to_i})", GitolitePublicKey::STATUS_ACTIVE],
                        ["#{l(:status_locked)} (#{key_count_by_active[0].to_i})", GitolitePublicKey::STATUS_LOCKED]], selected)
  end
